...
def eval_node(environment, node):
    if node.isatom():
        ...
        print(
            "OP: '%s' '%s' %s" % (str(operator), node.get_item(0)._typ, type(operator))
        )
        if operator in Builtin:
            ...
            if nargs < args_expected:
                ...
                if nargs == 1:
                    suffix = ""
                print(
                    "Unable to evaluate '%s' - needs more than %d argument%s"
                    % (op_name, nargs, suffix)
                )
                return 0
            if operator in Builtin:
...
def apply_op(environment, op, all_args):
    if op == Builtin.OP_ASSIGN:
        ...
        val_atom = eval_node(
            environment, all_args[1]
        )  # should return a NewAtom-INTEGER(17), not an EnvItem
