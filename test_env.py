...
class TestEnvItem:
    def test_env_number_item(self):
        num_item = EnvItem("v-number", ItemType.INTEGER, (30,))
        got = str(num_item)
        want = 'v-number: %i' % 30
        assert got == want

    def test_env_empty_number_item(self):
        num_item = EnvItem("v-number", ItemType.INTEGER)
        got = str(num_item)
        want = 'v-number: %i' % 0
        assert got == want

    def test_env_text_item(self):
        text_item = EnvItem("v-text", ItemType.TEXT, ("abc",))
        got = str(text_item)
        want = "v-text: '%s'" % 'abc'
        assert got == want

    def test_env_empty_text_item(self):
        text_item = EnvItem("v-text", ItemType.TEXT)
        got = str(text_item)
        want = "v-text: '%s'" % ''
        assert got == want

    def test_env_nil_item(self):
        nil_item = EnvItem("nil", ItemType.NIL)
        got = str(nil_item)
        want = 'nil'
        assert got == want

    def test_env_nil_creation(self):
        with pytest.raises(Exception):
            nothing_item = EnvItem("nothing", ItemType.NIL)
...
def root_env():
    e = EnvTable()
    num_item = EnvItem("a-number", ItemType.INTEGER, (30,))
    some_num_item = EnvItem("some-value", ItemType.INTEGER, (794,))
    text_item = EnvItem("a-text", ItemType.TEXT, ("abc",))
    e.set_item(num_item)
...
def child_env(root_env):
    e = EnvTable(root_env)
    another_num_item = EnvItem("another-number", ItemType.INTEGER, (19,))
    another_text_item = EnvItem("another-text", ItemType.TEXT, ("xyz reading",))
    override_num_item = EnvItem("v-number", ItemType.INTEGER, (1014,))
    some_text_item = EnvItem("some-value", ItemType.Text, ("textual now"))                                                              
...
class TestEnvTable:
    ...
    def test_add_nil_item(self, root_env):
        ...
        root_env.set_item(EnvItem("nil", ItemType.NIL))
